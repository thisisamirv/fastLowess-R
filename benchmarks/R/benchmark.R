...
#'
#' Using stats::lowess (1D, degree 1) to match 'lowess' crate capabilities.
#'
...
# ============================================================================
# Benchmark Categories (Using stats::lowess)
# ============================================================================
...
benchmark_scalability <- function(iterations = 10) {
  ...
  for (size in sizes) {
    ...
    run <- function() {
      lowess(data$x, data$y, f = 0.1, iter = 3) # iter=3 standard
    }
...
benchmark_delta <- function(iterations = 10) {
  results <- list()
  size <- 10000
  data <- generate_sine_data(size)

  # Configs: name -> delta value
  # Note: Rust uses 0.0, 0.5, 2.0, 10.0
  # R default is 0.01 * range. Range of sine data x is [0, 10]. So default is 0.1.
  ...
  for (name in names(delta_values)) {
    d <- delta_values[[name]]
    run <- function() {
      lowess(data$x, data$y, f = 0.2, iter = 2, delta = d)
    }
    results[[paste0("delta_", name)]] <- run_benchmark(paste0("delta_", name), size, run, iterations)
  }
  results
}
...
benchmark_fraction <- function(iterations = 10) {
  ...
  for (frac in fractions) {
    run <- function() {
      lowess(data$x, data$y, f = frac, iter = 3)
    }
...
benchmark_iterations <- function(iterations = 10) {
  ...
  for (it in iter_values) {
    run <- function() {
      lowess(data$x, data$y, f = 0.2, iter = it)
    }
...
benchmark_financial <- function(iterations = 10) {
  ...
  for (size in sizes) {
    ...
    run <- function() {
      lowess(data$x, data$y, f = 0.1, iter = 3)
    }
...
benchmark_scientific <- function(iterations = 10) {
  ...
  for (size in sizes) {
    ...
    run <- function() {
      lowess(data$x, data$y, f = 0.15, iter = 3)
    }
...
benchmark_genomic <- function(iterations = 10) {
  ...
  for (size in sizes) {
    ...
    run <- function() {
      lowess(data$x, data$y, f = 0.1, iter = 3, delta = 100.0)
    }
...
benchmark_pathological <- function(iterations = 10) {
  ...
  run_clustered <- function() {
    lowess(data_clustered$x, data_clustered$y, f = 0.3, iter = 3)
  }
  ...
  run_noise <- function() {
    lowess(data_noisy$x, data_noisy$y, f = 0.5, iter = 3)
  }
  ...
  run_outliers <- function() {
    lowess(data_outlier$x, data_outlier$y, f = 0.2, iter = 11)
  }
  ...
  run_const <- function() {
    lowess(data_const$x, data_const$y, f = 0.2, iter = 3)
  }
...
main <- function() {
  cat("============================================================================\n")
  cat("R LOWESS BENCHMARK SUITE (stats::lowess)\n")
  cat("============================================================================\n")
  ...
  all_results$delta <- unname(benchmark_delta(iterations))
  ...
  # Note: polynomial and dimensions benchmarks removed as lowess is strictly 1D linear

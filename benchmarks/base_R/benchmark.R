...
# Base R lowess benchmark runner with JSON output for comparison.
# This replicates the benchmarks/fastLowess/benchmark.py (and .R) examples
# but targets the built-in stats::lowess function.
...
generate_data_with_outliers <- function(size) {
    ...
    x <- data$x
    y <- sin(x) # Base y is just sin(x) for outliers case
...
main <- function() {
    ...
    out_path <- file.path(out_dir, "base_R_benchmark.json")

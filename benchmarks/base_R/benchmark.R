#!/usr/bin/Rscript
# Industry-level LOWESS benchmarks for base R (stats::lowess).
# Aligned with the fastLowess-R benchmarks for direct comparison.
# Results are written to benchmarks/output/base_R_benchmark.json.
#
...
# ============================================================================
# Data Generation (Aligned with Python and fastLowess-R)
# ============================================================================
...
main <- function() {
  ...
  cat("BASE R LOWESS BENCHMARK SUITE\n")
  cat(paste(rep("=", 80), collapse = ""), "\n")
  ...
  # 1. Scalability
  cat("\nSCALABILITY\n")
  sizes <- c(1000, 5000, 10000, 20000) # stats::lowess is too slow for 100k
  scale_results <- list()
  for (size in sizes) {
    data <- generate_sine_data(size)
    res <- bench::mark(
      stats::lowess(data$x, data$y, f = 0.1, iter = 3L),
  ...
  for (f in fractions) {
    res <- bench::mark(
      stats::lowess(data$x, data$y, f = f, iter = 3L),
      iterations = iterations, check = FALSE
    )
  ...
  for (it in iters) {
    res <- bench::mark(
      stats::lowess(data$x, data$y, f = 0.2, iter = as.integer(it)),
      iterations = iterations, check = FALSE
    )
  ...
  for (d in deltas) {
    res <- bench::mark(
      stats::lowess(data$x, data$y, f = 0.2, iter = 2L, delta = d[[2]]),
      iterations = iterations, check = FALSE
    )
  ...
  for (size in c(500, 1000, 5000, 10000)) {
    ...
    res <- bench::mark(
      stats::lowess(data$x, data$y, f = 0.1, iter = 2L),
      iterations = iterations, check = FALSE
    )
  ...
  for (size in c(500, 1000, 5000, 10000)) {
    ...
    res <- bench::mark(
      stats::lowess(data$x, data$y, f = 0.15, iter = 3L),
      iterations = iterations, check = FALSE
    )
  ...
  for (size in c(1000, 5000, 10000)) {
    ...
    res <- bench::mark(
      stats::lowess(data$x, data$y, f = 0.1, iter = 3L, delta = 100.0),
      iterations = iterations, check = FALSE
    )
  ...
  res <- bench::mark(stats::lowess(data$x, data$y, f = 0.3, iter = 2L), iterations = iterations, check = FALSE)
  ...
  res <- bench::mark(stats::lowess(data$x, data$y, f = 0.5, iter = 5L), iterations = iterations, check = FALSE)
  ...
  res <- bench::mark(stats::lowess(data$x, data$y, f = 0.2, iter = 10L), iterations = iterations, check = FALSE)
  ...
  res <- bench::mark(stats::lowess(data$x, data$y, f = 0.2, iter = 2L), iterations = iterations, check = FALSE)
  ...
  out_path <- file.path(final_output_dir, "base_R_benchmark.json")

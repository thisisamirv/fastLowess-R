...
example_1_basic_smoothing <- function() {
    ...
    cat("First 5 smoothed values:", paste(round(result$y[1:5], 6), collapse = ", "), "\n\n")
}
...
example_2_with_intervals <- function() {
    ...
    for (i in seq_along(x)) {
        ci_lower <- if (!is.null(result$confidence_lower)) result$confidence_lower[i] else 0
        ci_upper <- if (!is.null(result$confidence_upper)) result$confidence_upper[i] else 0
...
example_4_cross_validation <- function() {
    cat("Example 4: Cross-Validation for Fraction Selection\n")
    cat(strrep("-", 80), "\n")

...
example_2_sensor_simulation <- function() {
    ...
    for (i in 1:min(10, length(result$y))) {
        cat(sprintf("%6.0f %10.2f°C %10.2f°C\n", x[i], y[i], result$y[i]))
    }
...
# =============================================================================
example_3_window_size_comparison <- function() {
    ...
    for (window_size in window_sizes) {
        ...
        if (length(result$y) >= 5) {
            ...
            cat("  Last 5 smoothed:", paste(round(last_5, 4), collapse = ", "), "\n")
        } else {
            cat("  Smoothed values:", paste(round(result$y, 4), collapse = ", "), "\n")
        }
    }
...
example_4_memory_bounded <- function() {
    ...
    total_points <- 10000
    cat(sprintf("Processing %d data points with minimal memory footprint...\n", total_points))

    x <- as.numeric(0:(total_points - 1))
    y <- 2.0 * x + sin(x * 0.1) * 5.0 + ((0:(total_points - 1)) %% 7 - 3.0) * 0.5

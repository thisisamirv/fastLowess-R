...
example_1_basic_streaming <- function() {
    ...
    cat("First 5 smoothed values:", paste(round(result$y[1:5], 4), collapse = ", "), "\n\n")
}
...
example_4_parallel_comparison <- function() {
    ...
    # Parallel execution
    start_time <- Sys.time()
    result_parallel <- smooth_streaming(
        x, y,
        fraction = 0.5,
        chunk_size = 1000L,
        overlap = 100L,
    ...
    start_time <- Sys.time()
    result_sequential <- smooth_streaming(
        x, y,
        fraction = 0.5,
        chunk_size = 1000L,
        overlap = 100L,

test_that("smooth() basic functionality works", {
  x <- c(1, 2, 3, 4, 5)
  y <- c(2, 4, 6, 8, 10)

  result <- smooth(x, y, fraction = 0.67)
...
test_that("smooth() handles different fractions", {
  set.seed(42)
  x <- seq(0, 10, length.out = 50)
  y <- sin(x) + rnorm(50, sd = 0.1)

  result_low <- smooth(x, y, fraction = 0.2)
  result_high <- smooth(x, y, fraction = 0.8)
...
test_that("smooth() robustness iterations work", {
  set.seed(123)
  x <- seq(0, 10, length.out = 50)
  y <- sin(x) + rnorm(50, sd = 0.1)
  ...
  result_no_robust <- smooth(x, y, fraction = 0.3, iterations = 0)
  result_robust <- smooth(x, y, fraction = 0.3, iterations = 5)

  expect_equal(length(result_no_robust$y), length(y))
  expect_equal(length(result_robust$y), length(y))
})

test_that("smooth() confidence intervals work", {
  set.seed(42)
  x <- seq(0, 10, length.out = 50)
...
test_that("smooth() prediction intervals work", {
  set.seed(42)
  x <- seq(0, 10, length.out = 50)
  y <- sin(x) + rnorm(50, sd = 0.2)
  ...
  result_ci <- smooth(x, y, fraction = 0.5, confidence_intervals = 0.95)
  expect_true(
...
test_that("smooth() diagnostics work", {
  set.seed(42)
  x <- seq(0, 10, length.out = 50)
...
test_that("smooth() residuals work", {
  set.seed(42)
  x <- seq(0, 10, length.out = 50)
  y <- sin(x) + rnorm(50, sd = 0.1)
...
test_that("smooth() robustness weights work", {
  set.seed(42)
  x <- seq(0, 10, length.out = 50)
  y <- sin(x) + rnorm(50, sd = 0.1)
  ...
  expect_true(result$robustness_weights[25] < median(result$robustness_weights))
})

test_that("smooth() cross-validation works", {
  set.seed(42)
...
test_that("smooth() handles edge cases", {
  # Minimum data points
  x <- c(1, 2, 3)
  y <- c(1, 2, 3)
  result <- smooth(x, y, fraction = 0.67)
...
test_that("smooth() parameter validation works", {
  x <- 1:10
  y <- 1:10

  # Mismatched lengths
  expect_error(smooth(1:10, 1:5))
})

test_that("smooth() parallel execution works", {
  set.seed(42)
  x <- seq(0, 10, length.out = 1000)
  y <- sin(x) + rnorm(1000, sd = 0.1)

  result_serial <- smooth(x, y, fraction = 0.3, parallel = FALSE)
  result_parallel <- smooth(x, y, fraction = 0.3, parallel = TRUE)

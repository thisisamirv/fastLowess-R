test_that("smooth_online() basic functionality works", {
    set.seed(42)
    x <- 1:100
    y <- sin(x / 10) + rnorm(100, sd = 0.1)

    result <- smooth_online(x, y,
...
test_that("smooth_online() window capacity works", {
    set.seed(42)
    x <- 1:100
    y <- sin(x / 10) + rnorm(100, sd = 0.1)

    result_small <- smooth_online(x, y, fraction = 0.3, window_capacity = 15)
    result_large <- smooth_online(x, y, fraction = 0.3, window_capacity = 50)

    expect_equal(length(result_small$y), length(y))
    expect_equal(length(result_large$y), length(y))
})

test_that("smooth_online() min_points parameter works", {
    set.seed(42)
    x <- 1:50
...
test_that("smooth_online() update modes work", {
    set.seed(42)
    x <- 1:100
    y <- sin(x / 10) + rnorm(100, sd = 0.1)

    result_full <- smooth_online(x, y,
        fraction = 0.3, window_capacity = 25,
        update_mode = "full"
    )
    result_incr <- smooth_online(x, y,
        fraction = 0.3, window_capacity = 25,
...
test_that("smooth_online() handles edge cases", {
    # Minimum data points
    x <- 1:10
    y <- 1:10
    result <- smooth_online(x, y,
    ...
    result2 <- smooth_online(x, y,
        fraction = 0.5, window_capacity = 20, min_points = 3
    )
    expect_equal(length(result2$y), 10)
})

test_that("smooth_online() robustness works", {
    set.seed(42)
    x <- 1:100
    y <- sin(x / 10) + rnorm(100, sd = 0.1)
    y[50] <- y[50] + 5 # Add outlier

    result_no_robust <- smooth_online(x, y,
        fraction = 0.3, window_capacity = 25,
        iterations = 0
    )
    result_robust <- smooth_online(x, y,
        fraction = 0.3, window_capacity = 25,
        iterations = 3
    )

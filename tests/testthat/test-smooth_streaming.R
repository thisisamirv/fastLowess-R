test_that("smooth_streaming() basic functionality works", {
    set.seed(42)
    x <- seq(0, 10, length.out = 1000)
    y <- sin(x) + rnorm(1000, sd = 0.1)

    result <- smooth_streaming(x, y,
...
test_that("smooth_streaming() handles different chunk sizes", {
    set.seed(42)
    x <- seq(0, 10, length.out = 500)
    y <- sin(x) + rnorm(500, sd = 0.1)

    result_small <- smooth_streaming(x, y, fraction = 0.3, chunk_size = 100)
    result_large <- smooth_streaming(x, y, fraction = 0.3, chunk_size = 250)
...
test_that("smooth_streaming() overlap parameter works", {
    set.seed(42)
    x <- seq(0, 10, length.out = 500)
    y <- sin(x) + rnorm(500, sd = 0.1)

    result_no_overlap <- smooth_streaming(x, y,
        fraction = 0.3, chunk_size = 100, overlap = 0
    )
    result_overlap <- smooth_streaming(x, y,
...
test_that("smooth_streaming() diagnostics work", {
    set.seed(42)
    x <- seq(0, 10, length.out = 500)
...
test_that("smooth_streaming() handles edge cases", {
    # Small dataset
    x <- 1:50
    y <- sin(x / 10)
    result <- smooth_streaming(x, y, fraction = 0.3, chunk_size = 20)
    expect_equal(length(result$y), 50)

    # Chunk size larger than data
    result2 <- smooth_streaming(x, y, fraction = 0.3, chunk_size = 100)
    expect_equal(length(result2$y), 50)
})

test_that("smooth_streaming() parallel execution works", {
    set.seed(42)
    x <- seq(0, 10, length.out = 1000)
    y <- sin(x) + rnorm(1000, sd = 0.1)

    result_serial <- smooth_streaming(x, y,
        fraction = 0.3, chunk_size = 200, parallel = FALSE
    )
    result_parallel <- smooth_streaming(x, y,

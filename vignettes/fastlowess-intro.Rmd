---
title: "Introduction to fastlowess"
author: "Amir Valizadeh"
date: "`r Sys.Date()`"
output: BiocStyle::html_document
vignette: >
  %\VignetteIndexEntry{Introduction to fastlowess}
...
`fastlowess` is a high-performance R package for LOWESS (Locally Weighted Scatterplot Smoothing) built on a Rust backend. It provides significant speed improvements over standard R implementations while offering advanced features like robustness iterations, confidence intervals, and streaming processing.

## What is LOWESS?

LOWESS is a nonparametric regression method that fits smooth curves through scatter plots by fitting weighted polynomials at each point using nearby data. Key advantages include:
...
# Robust smoothing
result_robust <- smooth(x, y_outliers, fraction = 0.3, iterations = 5)
...
LOWESS is commonly used for smoothing genomic data such as copy number variation, methylation profiles, or ChIP-seq signals.
...
# Streaming smoothing
result_stream <- smooth_streaming(
  x_large, y_large,
  fraction = 0.1,
  chunk_size = 5000,
  overlap = 500
...
`fastlowess` provides significant speed improvements over standard implementations:
...
For detailed benchmarks, see the [package repository](https://github.com/thisisamirv/fastLowess-R/tree/bench/benchmarks).
